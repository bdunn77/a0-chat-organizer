...
def check_plugin() -> None:
    ...
    require("function collectDescendantChatIds(" in store, "plugin collects descendant chats for parent delete")
    ...
    require((ROOT / "extensions" / "python" / "startup_migration" / "_40_purge_stranded_chats.py").is_file(), "plugin purges leftover children of deleted parents")
    require("function collectFamilyChatIds(" in store, "plugin collects parent/child families for folder moves")
    require("syncFamilyMembership(" in store, "plugin keeps parent/child families in the same folder")
    require("_sync_family_membership" in backend, "backend keeps parent/child families in the same folder")
    require("_family_ids_for_move" in backend, "backend files the whole parent/child family together")
